"""
This script casts transects along the designated  stream file at a specified distance,
with the transectss length being some function of the bankfull width.
"""
...
# Set local variables
env.workspace = "C:\Users\graham.farley\Downloads\Data\MountainStreams.gdb" #Change according to the ecoregion
#Lines="R:\_GIS_Data\Site_Search_Project\Data\StreamsByHUC8.gdb\Lynches"
SplitType="Split at approximate distance"
DistanceSplit=20.0 #Distance between transects
TransecLength=1.0  #Just a placeholder
TransecLength_Unit="FEET"
OutputTransectDatabase="C:\Users\graham.farley\Downloads\Data\MountainTransects.gdb"
...
"""
This function joins the attributes of the original streams to the new strem file
created by the transect tool. The transect tool creates a new stream file that where
each feature ends at the specified distance between transects. The join allows us to
use the drainage area of each stream to determine the length of the transects.
"""
...
def JoinDrainage(streams,linesplits):
    ...
    arcpy.SpatialJoin_analysis(targetFeatures,joinFeatures,outfc,"#","#",fieldmappings,"SHARE_A_LINE_SEGMENT_WITH")
    return outfc
...
for feature in fcs:
    Lines = os.path.join("C:\Users\graham.farley\Downloads\Data\MountainStreams.gdb",feature)
    print(Lines)
    OutputTransect = Lines+"_Transect"
    # Create "General" file geodatabase
    WorkFolder="C:\Users\graham.farley\Downloads\Data\Empty"
    ...
    LineSplit = JoinDrainage(Lines,LineSplit)


    """
    This part calculates the transect length by multiplying the bankfull width
    by ten, unless the bankfull width is less than 10ft, then no transect is cast.
    """
    with arcpy.da.UpdateCursor(LineSplit,["Bankfull_Width_ft","Distance"]) as cursor:
        k=0
        for row in cursor:
            if row[0]<= 10:
                row[1] = 0
